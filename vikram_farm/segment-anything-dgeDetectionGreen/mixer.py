...
def mixer_function(dataR, dataG, dataB,data_list):
    ...
    mixed_image = auto_canny_edge_detection(mixed_image)
    mixed_image=(np.stack((np.array(mixed_image),)*3,-1) * 255.999).astype(np.uint8)
    mixed_image[:,:,2] = np.zeros((637,902))
    mixed_image[:,:,0] = np.zeros((637,902))
