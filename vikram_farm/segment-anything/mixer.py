...
def mixer_function(data_list):
    ...
    # # Reshape the image data to have shape (n_samples, n_features)
    # n_samples, height, width, n_channels = image_data.shape
    # flattened_data = image_data.reshape(n_samples, height * width, n_channels)

    # # Reshape the data to have shape (n_samples, n_features)
    # flattened_data = flattened_data.reshape(n_samples, -1)
    ...
    # transformed_image_data = transformed_data.reshape(n_samples, height, width, 3)

    # # Convert the transformed data to an RGB image
    # transformed_image = Image.fromarray(np.uint8(transformed_image_data))
    ...
    red_array = data_list[0]
    green_array = data_list[1]
    blue_array = data_list[2]
    ...
    # canny edge overlaying---------------------------------
    # def auto_canny_edge_detection(image, sigma=0.33):
    #     md = np.median(image)
    #     lower_value = int(max(0, (1.0-sigma) * md))
    #     upper_value = int(min(255, (1.0+sigma) * md))
    #     return cv2.Canny(image, lower_value, upper_value).astype(np.uint8)
    
    # edge = auto_canny_edge_detection(mixed_image)
    # edge=(np.stack((np.array(edge),)*3,-1) * 255.999).astype(np.uint8)
    # edge[:,:,2] = np.zeros((637,902))
    # edge[:,:,0] = np.zeros((637,902))

    # mixed_image = cv2.addWeighted(mixed_image, 0.8, edge, 0.2, 0)
     # canny edge overlaying---------------------------------
